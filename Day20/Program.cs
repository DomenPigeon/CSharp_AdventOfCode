...
namespace Day20 {
    internal static class Program {
        ...
        private static List<int> FindLenghts(string input) {
            var insideParenthesis = "";
            var beforeParenthesis = CutBeforeParenthesis(input, ref insideParenthesis);
            var end               = "";
            insideParenthesis = InsideParenthesis(insideParenthesis, ref end);

            var startDistances = new List<int> {Distance(beforeParenthesis)};
            var part1Distances = new List<int>();
            var part2Distances = new List<int>();

            var part2 = "";
            var part1 = FindParts(insideParenthesis, ref part2);

            var rest = "";
            if (CutBeforeParenthesis(part1, ref rest) == part1)
                part1Distances.Add(Distance(part1));
            else
                part1Distances = FindLenghts(part1);

            if (CutBeforeParenthesis(part2, ref rest) == part2)
                part2Distances.Add(Distance(part2));
            else
                part2Distances = FindLenghts(part2);

            var result = new List<int>();
            foreach (var distance in startDistances) {
                foreach (var otherDistance in part1Distances)
                    result.Add(otherDistance + distance);

                foreach (var part2Distance in part2Distances)
                    result.Add(part2Distance + distance);
            }
            ...
            var endDistances = FindLenghts(end);

            if (end[0] == '(') {
                foreach (var res in result) {
                    foreach (var distance in endDistances)
                        newResult.Add(res + distance);
                }
            }
            else {
                foreach (var res in startDistances) {
                    foreach (var distance in endDistances)
                        newResult.Add(res + distance);
                }
            }
        ...
        private static string FindParts(string insideParenthesis, ref string part2) {
            var level = 0;
            for (var i = 0; i < insideParenthesis.Length; i++) {
                if (insideParenthesis[i] == '(') level++;
                if (insideParenthesis[i] == ')') level--;

                if (insideParenthesis[i] == '|' && level == 0) {
                    var part1 = insideParenthesis.Substring(0, i);
                    part2 = insideParenthesis.Substring(i + 1);
                    return part1;
                }
            }
        ...
        private static string CutBeforeParenthesis(string input, ref string rest) {
        ...
        private static string InsideParenthesis(string parenthesis, ref string rest) {
            var level = 0;
            for (var i = 0; i < parenthesis.Length; i++) {
                if (parenthesis[i] == '(') level++;
                if (parenthesis[i] == ')') {
                    level--;
                    if (level == 0) {
                        rest = parenthesis.Substring(i + 1);
                        return parenthesis.Substring(1, i - 1);
                    }
                }
            }
        ...
        private static int Distance(string path) {
            if (path.Length % 2 != 0) return path.Length;

            var middle = path.Length / 2;
            for (var i = 0; i < path.Length - middle; i++) {
                if (path[middle - i - 1] != BackSymbol(path[middle + i])) return path.Length;
            }

            return middle;
        }

        private static char BackSymbol(char symbol) {
            if (symbol == 'N') return 'S';
            if (symbol == 'S') return 'N';
            if (symbol == 'W') return 'E';
            if (symbol == 'E') return 'W';
            return '?';
        }

        private static List<List<T>> Clone2DList<T>(IEnumerable<List<T>> listToClone) {
        ...
        private static void ConsoleWrite2DArray<T>(IEnumerable<List<T>> _2DArray) {
            var i = 0;
